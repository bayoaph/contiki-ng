...
PROCESS_THREAD(led_blink_process, ev, data)
{
  ...
  while (1)
  {
    ...
    // Exercise 1b: Uncommenting ff three lines infinitely turns on led light
    leds_off(LEDS_RED);
...
PROCESS_THREAD(hello_world_process, ev, data)
{
  ...
  // static int counter = 0; // Track of the number of "Hello, world" prints (Used in Exercise 1d)
  ...
  while (1)
  { 
    // Exercise 1d: Stops after 10 prints
    // if (counter >= 10) 
    //   break;

    printf("Hello, world\n");
    // counter++; (Used in Exercise 1d) Increment number of prints

    etimer_set(&timer_wait, CLOCK_SECOND * 10); // Exercise 1c: Changing value from 10 to 4 changes delay interval
    PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&timer_wait));
  }
...
OBSERVATION:
Uncommenting the three lines in led_blink_process, specifically the part where we turn off the led,
makes the led turn on non-stop. Although we needed to remove the declared timer_wait because 
it was and unused at the time and causes compilation errors.
...
PROCESS_THREAD(hello_world_process, ev, data)
{
  ...
  { 
    ...
    etimer_set(&timer_wait, CLOCK_SECOND * 4); // Changing value from 10 to 4 changes delay interval
    PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&timer_wait));
  }
...
/* ADDITIONAL NOTE:
 * For some reason, there are issues on using blue led.
 * Hence, red led was used as substitute, since it works well.
 */
