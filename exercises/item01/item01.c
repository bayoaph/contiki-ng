 ...
 *Changes are below
  PROCESS_THREAD(hello_world_process, ev, data)
  {
    ...
    etimer_set(&timer_startup, CLOCK_SECOND * 30); // Startup delay
    PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&timer_startup));

    while (1) // Infinite loop
    {
      printf("Hello, world\n"); // Print "Hello, world"
      etimer_set(&timer_wait, CLOCK_SECOND * 4); // 4-second delay
      PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&timer_wait));
    }
...
/* Exercise 1d: alter the program such that the node stops printing "Hello World" after 10 times'
  *Changes are below
  *add a counter and change the condition of the while loop to counter < 10
  PROCESS_THREAD(hello_world_process, ev, data)
  {
    PROCESS_BEGIN();
    static struct etimer timer_startup, timer_wait;
 ...
 *Comment out the led blink process and incorporate an led blink in the hello world process
    PROCESS_THREAD(hello_world_process, ev, data)
    {
      PROCESS_BEGIN();
      static struct etimer timer_startup, timer_wait, led_timer;
 ...
 *Led Blink Process(a)
  ...
  *Hello World Process(b)
    PROCESS_THREAD(hello_world_process, ev, data)
    {
      PROCESS_BEGIN();
      static struct etimer timer_startup, timer_wait;

      etimer_set(&timer_startup, CLOCK_SECOND * 30); // After 30 seconds Hello World stars printing
      PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&timer_startup));

      while (1)
      {
        printf("Hello, world\n");
        etimer_set(&timer_wait, CLOCK_SECOND * 10); //Every 10 seconds Hello World Prints
        PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&timer_wait));
      }
