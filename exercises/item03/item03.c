 ...
 *
 * Each node checks if it has a temperature sensor, and if it does, it reads the temperature and creates a message with it. 
 * If the node doesn’t have a sensor, it sends a message saying it doesn’t have one. This message, including the temperature 
 * if available, is sent as a broadcast using a structure called temperatureMessage. When another node receives this broadcast, 
 * it prints the message and temperature to the serial output and blinks its red LED for 1 second. The sending node blinks its
 * blue LED to show it sent something. This way, the nodes are working together, exchanging information in the network. 
 * The code pretty much handles all these steps correctly.
 *
 */
