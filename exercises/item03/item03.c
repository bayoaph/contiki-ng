...
The code implemented Contiki OS application for broadcasting temperature data over UDP and receiving it at other nodes in a network. 
Using the UDP Communications, UDP protocol to broadcast sensor data (temperature) to all nodes in a network.
The send_broadcast_packet function sends a UDP packet containing the temperatureMessage in every 4 seconds the simulator 
printed "sent broadcast message," broadcast packet is sent to all nodes, and the LED is turned on for 1 second.
 */
 ...
 * 
 * For this exercise, I use sky mote and use the sensor sht11.
 * 
 * The nodes with sht11 sensor that measures temperature, technically it has a sht11_sensor.value(SHT11_SENSOR_TEMP)
 * and package the data into a temperatureMessage struct. This node sent the tempearture to the other node, the simulator
 * print the 2 nodes results were the first node print that indicates send the temperature with the temperature value. The other node
 * that dont have a sensor print no value.
 *
 * MODIFIED CODE:
 */
// #include "contiki.h"
...
// //#include "dev/zoul-sensors.h"
// #include "dev/sht11-sensor.h" // Using sht11 sensor instead of zoul
