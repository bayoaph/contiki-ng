 ...
 */
  /**************************************************************************************************
  Initially, the program did not run successfully, and I struggled to understand its functionality, 
  particularly the broadcasting and callback mechanisms. After resolving the issues and studying the 
  code, I ran it on Sky motes. Nodes with an SHT11 sensor broadcasted temperature readings, while 
  others sent messages indicating no sensor. Received broadcasts were displayed with temperature, 
  sender address.
 **************************************************************************************************/
 ...
 */
  /**************************************************************************************************
  Compiling and running the program encountered minor adjustments due to unavailable blue LEDs, 
  leading to the use of the red LED instead. After successfully deploying the code on Sky motes, 
  the program broadcasted messages periodically. For nodes with a temperature sensor, messages such 
  as "Here is node X, sending the temperature" were sent, with temperature readings like 6360
  displayed on the serial output. Nodes without sensors sent "Here is node X, I don't have a sensor" 
  with a temperature value of 0. Upon receiving a broadcast, other nodes displayed the message and 
  blinked the red LED for one second.
