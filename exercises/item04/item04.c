...
/* Timer callback to turn off the green LED */
static void
...
void
construct_ipv6_address(uip_ipaddr_t *addr, uint16_t node_id) {
  linkaddr_t target_linkaddr;

  /* This logic assumes a particular node layout and increments/decrements
     linkaddr bytes to find a "neighbor" node. Adjust as necessary. */
  if (node_id % 2 == 0) {
...
udp_rx_callback(struct simple_udp_connection *c,
  ...
  // If not from this node, send it back
  if (tmReceived.originator != node_id) {
    ...
    // If it's our own packet returning, calculate RTT
    rtt -= tmReceived.time;
