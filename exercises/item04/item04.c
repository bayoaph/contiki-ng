...
/*---------------------------------------------------------------------------*/
PROCESS(rtt_simple_udp_process, "RTT using Simple UDP");
AUTOSTART_PROCESSES(&rtt_simple_udp_process);
/*---------------------------------------------------------------------------*/
...
static void udp_recv_callback(struct simple_udp_connection *c,
  ...
  printf("\nTime received: %lu clock ticks = %lu secs %lu millis\n",
  ...
  /* Check if the packet is from another node */
...
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(rtt_simple_udp_process, ev, data) {
  PROCESS_BEGIN();
  ...
  while (1) {
    ...
    /* Capture current time and node ID */
    tmSent.time = clock_time();
    tmSent.originator = node_id;

    /* Set destination address to link-local all-nodes multicast */
    uip_ipaddr_t addr;
