...
/*---------------------------------------------------------------------------*/
PROCESS(example_unicast_process, "RTT");
...
static void recv_uc(struct simple_udp_connection *c,
  ...
  // printf("unicast message received from ", from->u8[0], from->u8[1]);
...
PROCESS_THREAD(example_unicast_process, ev, data)
{
  ...
  PROCESS_BEGIN();
  simple_udp_register(&udp_conn, UDP_PORT, NULL, UDP_PORT, recv_uc);
